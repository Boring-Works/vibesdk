...
export function getProviderInfo(modelValue?: string): ProviderInfo {
	...
	if (modelValue.includes('granite')) {
		return { name: 'IBM Granite', color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' };
	}
	if (modelValue.includes('mistral')) {
		return { name: 'Mistral', color: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400' };
	}
	if (modelValue.includes('deepseek')) {
		return { name: 'DeepSeek', color: 'bg-rose-100 text-rose-800 dark:bg-rose-900/20 dark:text-rose-400' };
	}
