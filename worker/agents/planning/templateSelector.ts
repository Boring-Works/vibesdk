...
async function predictProjectType(
    ...
    try {
        ...
        const systemPrompt = `You are an expert project type classifier. Your task is to analyze user requests and determine what type of project they want to build.
...
function getSystemPromptForProjectType(projectType: ProjectType): string {
    if (projectType === 'app') {
        ...
        return `You are an expert software architect specializing in template selection for rapid development. Your task is to select the most suitable starting template based on user requirements.
...
- Provide clear, specific reasoning for selection`;
    ...
    return `You are an expert template selector. Your task is to select the most suitable ${projectType} template based on user requirements.
